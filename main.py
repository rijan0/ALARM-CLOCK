...
from kivy.core.audio import SoundLoader  # Import SoundLoader to load and play sound
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Add Image of person waking up (centered in the layout)
        image = Image(source='person_waking_up.png', size_hint=(None, None), size=(350, 350), pos_hint={'x': 0.12, 'y': 0.3})
        image.center = layout.center  # Center the image in the layout
        layout.add_widget(image)

        # Add Logo Image near the top
        logo = Image(source='LOGO.png', size_hint=(None, None), size=(350, 350), pos_hint={'x': 0.1, 'y': 0.65})
        layout.add_widget(logo)

        # Add "Get Started" button at the bottom center
        start_button = Button(
            text="GET STARTED",
            size_hint=(None, None),
            size=(300, 60),
            pos_hint={'x': 0.16, 'y': 0.05},  # Adjust position at the bottom center
            background_normal='',  # Transparent background
            background_color=(0, 0, 0, 0),  # No background color (transparent)
            color=(1, 1, 1, 1),  # White text color
            font_size='24sp'  # Larger font size
        )

        # Add custom white rounded border using Canvas
        with start_button.canvas.before:
            Color(0.3, 0.3, 0.3, 1)  # White color for border
            self.border = RoundedRectangle(
                size=start_button.size,
                pos=start_button.pos,
                radius=[25]  # Adjust the radius for rounded corners
            )
...
class NextScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Create ScrollView to make the content scrollable
        scroll_view = ScrollView(size_hint=(1, 1), pos_hint={'x': 0, 'y': 0})

        scroll_layout = FloatLayout(size_hint=(None, None), size=(Window.width, 1000))  # large enough size for scrolling

        # Add a label to indicate the alarm setting
        ...
        self.hour_spinner = Spinner(
            text='Hour',
            values=[str(i).zfill(2) for i in range(1, 13)],  # 1 to 12 for the hour
            size_hint=(None, None),
            size=(100, 50),
        ...
        self.minute_spinner = Spinner(
            text='Minute',
            values=[str(i).zfill(2) for i in range(0, 60, 1)],  # Minutes in 5-minute intervals
            size_hint=(None, None),
            size=(100, 50),
        ...
        set_alarm_button = Button(
            ...
            size_hint=(None, None),
            size=(300, 60),
            pos_hint={'x': 0.1, 'y': 0.3},  # Adjust position at the bottom center
            background_normal='',  # Transparent background
            background_color=(0, 0, 0, 0),  # No background color (transparent)
            color=(1, 1, 1, 1),  # White text color
            font_size='24sp'  # Larger font size
        ...
        # Update the border when the button size or position changes
        set_alarm_button.bind(pos=self.update_set_alarm_border, size=self.update_set_alarm_border)

        # Load the sound effect for the "Set Alarm" button
        self.button_click_sound = SoundLoader.load('button_click.mp3')  # Ensure you have this file
        ...
        # Bind the button to play sound and perform actions when clicked
        set_alarm_button.bind(on_press=self.set_alarm)

        scroll_layout.add_widget(set_alarm_button)

        # Add "Set Alarm Sound" button
        set_alarm_sound_button = Button(
            text="Set Alarm Sound",
            size_hint=(None, None),
            size=(300, 60),
            pos_hint={'x': 0.1, 'y': 0.2},  # Adjust position at the bottom center
            background_normal='',  # Transparent background
            background_color=(0, 0, 0, 0),  # No background color (transparent)
            color=(1, 1, 1, 1),  # White text color
            font_size='24sp'
    ...
    def set_alarm(self, instance):
        ...
        # Set the alarm time (today's date with the selected time)
        now = datetime.now()
    ...
    def set_alarm_sound(self, instance):
        # Open a file chooser dialog to select a sound file
        filechooser = FileChooserIconView()

        # Create a "Select" button within the popup
    ...
    def close_popup(self, instance=None):
        # Close the popup without affecting the app
        if self.popup:
            self.popup.dismiss()
    ...
    def set_selected_alarm_sound(self, selected_file):
        ...
        if selected_file:
            sound_path = selected_file[0]  # Get the selected sound file path
            self.button_click_sound = SoundLoader.load(sound_path)  # Load the selected sound
    ...
    def check_alarm(self, dt):
        ...
        if self.alarm_time and datetime.now() >= self.alarm_time:
            print("Alarm ringing...")
            if self.button_click_sound:
                self.button_click_sound.play()  # Play the alarm sound
            self.alarm_time = None  # Reset alarm time after it rings
